(function() {
    ...
    window.surfly_session_start = function () {
        if (window.CONTINUATION_POINT) {
            _get_continuation_url(window.location.href, function(continuation_url) {
                ...
                iframe = _create_iframe();
                iframe.src = PROXY_POINT+window.location.href;
        }
    ...
    function _create_iframe() {
        ...
        iframe.style.cssText = "position: fixed; top: 0px; left: 0px; display: none; overflow: auto; z-index: 6000";
    ...
    function _synchronize_iframe() {
        ...
        iframe.contentWindow.frames[0].postMessage(JSON.stringify(obj), "*");
    ...
    function _get_form_elements() {
        ...
        for (var i=0, n = forms.length; i<n; i++) {
            ...
            for (var j=0, m = children.length; j<m; j++) {
                ...
                if ((child.tagName == "OPTION") || (child.tagName == "INPUT")) {
    ...
    window.addEventListener('load', show_frame);
